...
class Strategy:
    ...
    def _train(self, epoch, loader_tr, optimizer):
        ...
        for batch_idx, (x, y, idxs) in enumerate(loader_tr):
            ...
            accFinal += torch.sum((torch.max(out, 1)
                                   [1] == y).float()).data.item()
            loss.backward()
    ...
    def train(self):
        ...
        while accCurrent < 0.99:
            ...
            epoch += 1
            print(str(epoch) + ' training accuracy: ' +
                  str(accCurrent), flush=True)
    ...
    def predict_prob(self, X, Y):
        loader_te = DataLoader(self.handler(
            X, Y, transform=self.args['transformTest']), shuffle=False, **self.args['loader_te_args'])
        self.clf.eval()
