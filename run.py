...
from query_strategies import RandomSampling, BadgeSampling, \
    ...
    AdversarialBIM, AdversarialDeepFool, WAAL
from utils import seed_everything
seed_everything(1106)

# code based on https://github.com/ej0cl6/deep-active-learning"
parser = argparse.ArgumentParser()
parser.add_argument('--alg', help='acquisition algorithm',
                    type=str, default='rand')
parser.add_argument(
    '--did', help='openML dataset index, if any', type=int, default=0)
parser.add_argument('--lr', help='learning rate', type=float, default=1e-4)
parser.add_argument(
    '--model', help='model - resnet, vgg, or mlp', type=str, default='mlp')
parser.add_argument('--path', help='data path', type=str, default='data')
parser.add_argument('--data', help='dataset (non-openML)',
                    type=str, default='')
parser.add_argument(
    '--nQuery', help='number of points to query in a batch', type=int, default=100)
parser.add_argument(
    '--nStart', help='number of points to start', type=int, default=100)
parser.add_argument(
    '--nEnd', help='total number of points to query', type=int, default=50000)
parser.add_argument(
    '--nEmb', help='number of embedding dims (mlp)', type=int, default=256)
parser.add_argument('-d', '--device', default='0', type=str,
                    help='indices of GPUs to enable (default: all)')
parser.add_argument('-n', '--experiment', required=False, default=9999, type=int,
                    help='experiment number')
opts = parser.parse_args()

os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
os.environ["CUDA_VISIBLE_DEVICES"] = f'{opts.device}'
...
wandb.init(project='badge', entity='visionnoob',
           name=f'ex_{opts.experiment}_{DATA_NAME}_{MODEL_NAME}_{NUM_QUERY}_{hex_id}')

wandb.config.NUM_INIT_LB = NUM_INIT_LB
wandb.config.NUM_QUERY = NUM_QUERY
wandb.config.NUM_ROUND = NUM_ROUND
wandb.config.DATA_NAME = DATA_NAME
...
args_pool['CIFAR10']['transform'] = args_pool['CIFAR10']['transformTest']
...
# badge but with k-DPP sampling instead of k-means++
    ...
    strategy = WAAL(X_tr, Y_tr, idxs_lb, net, handler, args)
    ...
    strategy = ActiveLearningByLearning(
        X_tr, Y_tr, idxs_lb, net, handler, args, strategy_list=albl_list, delta=0.1)
else:
...
print(f'ex_{1}_{hex_id}')

wandb.config.NUM_LABLED_POOL = NUM_INIT_LB
wandb.config.NUM_UNLABELED_POOL = n_pool - NUM_INIT_LB
wandb.config.NUM_TEST_POOL = n_test
...
wandb.log({'test/acc': acc[0],
           'test/n_query': NUM_INIT_LB})
for rd in range(1, NUM_ROUND+1):
    ...
    corr = (strategy.predict(X_tr[q_idxs], torch.Tensor(
        Y_tr.numpy()[q_idxs]).long())).numpy() == Y_tr.numpy()[q_idxs]
    ...
    wandb.log({'test/acc': acc[rd],
               'test/n_query': sum(idxs_lb)})
